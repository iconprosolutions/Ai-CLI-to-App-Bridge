...
function validateRoutes(data) {
  ...
  // Cross-engine fallbacks: overflowFallback (oversized-prompt policy) and
  // quotaFallback (engine-exhausted policy — every account cooling down or
  // logged out → the request moves engines instead of bouncing). Both must
  // name a real route on a DIFFERENT engine: same-engine failover is already
  // the account pool's job.
  const routeById = new Map(data.routes.map((r) => [r.id, r]));
  for (const r of data.routes) {
    for (const field of ['overflowFallback', 'quotaFallback']) {
      if (r[field] === undefined) continue;
      const target = typeof r[field] === 'string' ? routeById.get(r[field]) : null;
      if (!target) throw new Error(`routes.json: "${field}" must name an existing route id on ${r.id}`);
      if (target.id === r.id) throw new Error(`routes.json: "${field}" cannot point to itself on ${r.id}`);
      if (target.engine === r.engine) throw new Error(`routes.json: "${field}" on ${r.id} must target a different engine`);
    }
  }
